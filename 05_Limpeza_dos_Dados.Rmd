...
Aliás, a probabilidade da ocorrência de um dígito seria o logaritmo (base 10) do dígito maior menos o logaritmo (base 10) do dígito em si. A primeira versão na equação acima é a definição e a segunda, uma tranformação algébrica que facilita o cálculo.
...
1. Para uma distribuição univariada, ou seja, onde só tem uma varíavel numérica com valores faltando, a média dos dois valores em volta daquele que está faltando é uma boa escolha. Por exemplo, aqui se fazemos a média entre os casos da Rússia em 2004 e 2006, este resultado seria uma boa aproximação do valor que está faltando.
...
Precisou cinco modificações, mas chegamos de ter um conjunto de dados limpo e *tidy* e pronto a ser analisado. Os testes de outliers, Benford's Law, dados faltando, etc. são importante para processo de fazer modelos e análises que refletam corretamente as fenomena que você quer medir.