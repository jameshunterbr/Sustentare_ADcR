...
## *Tidy Data* -- Arrumar os Dados

Agora que temos dois objetos na memoria, `acoes` e `pac_data`, podemos proceder para limpar eles e prepará-los para as análises. Nesta fase do seu projeto, você vai começar de programar seriamente. Você vai combinar um número de funções em um script para preparar os dados para análise e modelagem. 

Primeiro, tem de entender algumas regras de sintaxe de R e algumas praxes para dar seu código uma aparência que é fácil de seguir e entender. 
...
### Limpando um Conjunto dos Dados 
...
O método de `dplyr` neste caso não é o mais eficiente (precisa 3 linhas de código), mas mostra dois pontos fortes do sistema de `tidyverse`. Primeiro, é um exemplo do uso do *pipe*, a ferramenta que vimos no último capitulo. Alguns programadores gostam de escrever a função `group_by()` no caso deste tibble assim:
 
 >  `flights %>% 
  group_by(., origin)`
  
com o ponto agindo para segurar o lugar do `flights` na função. Quando tiver 
