...
Primeiro desses funções permite que combinamos operações das funções diferentes na mesma expressão. Este símbolo está chamada o "pipe" [^6] e está escrito assim: `%>%`. Este conceito é muito comum em linguagens de computação, apesar que está expressa por vários símbolos. Em R, o pipe quer dizer "aceite o resultado da operação no lado esquerdo e aplique a função no lado direto ao este resultado". Com o uso do pipe, nós não precisamos repetir o nome do objeto do lado esquerdo da operação quando você especifica a função no lado direto. 

[^6]: Nenhum das palavras que traduz a palavra "*pipe*" realmente capta o sentido correto. Então, eu vou ficar com o inglês original.
...
### Novo Nomes para as Variáveis --- Loops
...
Vamos fazer um resumo dos dados e olhar em alguns gráficos para poder entender os dados melhor e ver se podemos tirar conclusões sobre o novo tratamento. 